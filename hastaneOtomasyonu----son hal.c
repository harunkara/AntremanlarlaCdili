...
void hastaSil(){
    	...
    	struct hasta hasta;
    	char aranan[11];
    	printf("Silinmesini istediðiniz hastanin TC'sini girin:");
    	scanf("%s",&aranan);
	    while(fscanf(hastaDosya, "%s%s%s%s%s%s%d",&hasta.tcno,&hasta.isim,&hasta.soyisim,&hasta.dgtarihi,&hasta.dgyeri,&hasta.kangrubu,&hasta.yas)!=EOF){
		if(strcmp(hasta.tcno,aranan)!=0)
...
while(J=1)
{
	...
	printf("HASTA KAYDI:1\nRANDEVU KAYDI:2\nMUAYENE KAYDI:\n");
    printf("HASTALAR:4\nRANDEVULAR:5\n");
    printf("MUAYENELER:6\n");
    printf("TC'YE GORE HASTA :7\nTC'YE GORE RANDEVU:8\n");
    printf("TC'YE GORE MUAYENE:9\n");
    printf("HASTA SIL:10\n");
    printf("RANDEVU SIL:11\n");
    printf("MUAYENE SIL:12\n");
